...
- **Backends (`browser.backend` setting).** The tool has two backends. `native` (default) is the Puppeteer/CDP Chromium described above and includes all `app.*` modes. `aside` (macOS-only) drives your **live logged-in Aside browser profile** via the Aside CLI — it is not isolated; commands run against your real signed-in session. Switch with the `/browser` selector. Explicit `app.path`/`app.browser`/`app.cdp_url` on `open` ALWAYS use `native`, regardless of the setting.
  - On the Aside backend, `run` exposes **Aside globals**, not Puppeteer: `page` is a Playwright-style Page (`page.goto/locator/getByRole/evaluate/waitForSelector/url()/title()/screenshot()`), plus `openTab(url)`, `closeTab`, `snapshot(page)` → `{ tree, refs, diff }`, and the coordinate API `cua` (`click/type/scroll/drag/getVisibleScreenshot()`). There is NO Puppeteer `browser`/`tab` helper on Aside — Puppeteer-only calls like `tab.goto`/`tab.id` are native-only. `act` verbs map onto Aside's `page` locators + `cua` and are addressed by `selector` (not observe `id`).
